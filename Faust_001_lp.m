...
fprintf('########################################################\n')
fprintf('#Reconstructing the Faust Dataset Scan 001 from supp fn evaluations#\n')
fprintf('########################################################\n\n')
rng(626)
...
load('faust scans\tr_scan_001_vertices.mat')
load('faust scans\tr_scan_001_random_unit_vectors.mat')
load('faust scans\tr_scan_001_support_result.mat')
%% Step 3: Regression
params = {};
...
for kk = 1 : 2
    ...
    for q = q_range
        ...
        pbaspect([1,2,1])
        view([144,12])
        filename = strcat('figures/Exp_Faust_001_lp',int2str(q),'_n',int2str(n));
        print(filename,'-dpdf')
 ...
 %%Step 3b : Least Squares Regression
    ...
    %% Save to disk
    cvxprinter_3d(A_LS);
    filename = strcat('figures/Faust/Faust 001/Exp_Faust_001_LS_n',int2str(m));
    print(filename,'-dpdf')