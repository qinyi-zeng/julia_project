...
fprintf('####################################################################\n')
fprintf('#Reconstructing the circle in R^2 from noiseless support fn evals.#\n')
fprintf('####################################################################\n\n')
rng(626)
...
d = 3;
sigma = 0.0;
[u_mat_all, h_vec_all] = gen_cvxsupp('circle',n);
...
cvxprinter_master(A_am,struct('savepath',strcat('figures/Exp_2d_circle_AM_nl',int2str(n))));
...
cvxprinter_master(A_LS,struct('savepath',strcat('figures/Exp_2d_circle_LSE_nl',int2str(n))));
