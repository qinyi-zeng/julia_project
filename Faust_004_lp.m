...
fprintf('########################################################\n')
fprintf('#Reconstructing the Faust Dataset Scan 004 from supp fn evaluations#\n')
fprintf('########################################################\n\n')
rng(626)
...
load('faust scans\tr_scan_004_vertices.mat')
load('faust scans\tr_scan_004_random_unit_vectors.mat')
load('faust scans\tr_scan_004_support_result.mat')
%% Step 3: Regression
params = {};
...
for kk = 1 : 2
    ...
    for q = q_range
        ...
        pbaspect([1,2,1])
        view([144,12])
        filename = strcat('figures/Faust/Faust 004/Exp_Faust_004_lp',int2str(q),'_n',int2str(n));
        print(filename,'-dpdf')
    ...
    %% Save to disk
    cvxprinter_3d(A_LS);
    filename = strcat('figures/Faust/Faust 004/Exp_Faust_004_LS_n',int2str(m));
    print(filename,'-dpdf')