...
fprintf('####################################################################\n')
fprintf('#Reconstructing the dodecahedron in R^3 from support fn evaluations.#\n')
fprintf('####################################################################\n\n')
rng(626)
...
d = 3;
sigma = 0.0;
[u_mat_all, h_vec_all] = gen_cvxsupp('dodecahedron',n);
...
cvxprinter_master(A_am,struct('savepath',strcat('figures/dode_nl_am',int2str(n))));
...
cvxprinter_master(A_LS,struct('savepath',strcat('figures/dode_nl_lse',int2str(n))));
